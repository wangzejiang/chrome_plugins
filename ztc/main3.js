function init3(){
	var mx = getMx("J_manage-adgroup-detail-list_cost_");
	//console.log("zejiang init3 ren qun-->"+mx.length);
	if(mx.length==0){
		window.setTimeout("init3()",1000);
		return;
	}
	var total_price = getMx("J_manage-adgroup-detail-list_total_cost");
	var total_count = getMx("J_manage-adgroup-detail-list_total_transactionshippingtotal");
	...
	mx.each(function(){
		...
		var id = $(this).attr('id');
		var wid = id.split('_')[3];
		var newid = "zejiang_"+wid;
		console.log("zejiang init3 ren qun id-->"+wid);
...
function getImpression3(id){
	var text1 = "J_manage-adgroup-detail-list_impression_"+id;
	return $("td[id='"+text1+"']");
}
function getMx(text1){
	return $("td[id^='"+text1+"']");
}
...
function getTransactionshippingtotal3(id){
	var text1 = "J_manage-adgroup-detail-list_transactionshippingtotal_"+id;
	var text = $("td[id='"+text1+"']").text();
	return val23(text);
}
function val23(val){
